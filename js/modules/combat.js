...
// 战斗系统 v6.9 (修复：战斗结束后禁止使用消耗品)
console.log("加载 战斗系统 (Item Usage Fix v6.9)");
...
const Combat = {
    ...
    start: function(enemyObj, onWin, logId, eToxBarId, eToxValId, pToxBarId, pToxValId) {
        ...
        if (this.enemy.toxAtk === undefined) this.enemy.toxAtk = 0;
        this.enemy.toxicity = 0;

        if (this.enemy.stats) {
            if (this.enemy.stats.toxAtk === undefined) this.enemy.stats.toxAtk = this.enemy.toxAtk;
            this.enemy.stats.toxicity = 0;
        }
    ...
    togglePause: function() {
        if (this.isStopped || this.isEnded) return; // 结束后也不能暂停/继续
    ...
    useConsumable: function(slotIndex) {
        ...
        // 【核心修改】检查战斗是否已结束
        if (this.isEnded) {
            if(window.showToast) window.showToast("战斗已结束，无法使用物品");
            return;
        }
    ...
    _performAttack: function(attackerName, atkStats, defStats, isPlayerAttacking) {
        ...
        if (Math.random() < dodgeRate) {
            ...
            const dodgeTooltip = `
                <div class="combat-tooltip-content">
                    <div class="tip-row"><span>攻方速度</span> <span>${spdAtk}</span></div>
                    <div class="tip-row"><span>守方速度</span> <span>${spdDef}</span></div>
                    <div class="tip-divider"></div>
                    <div class="tip-row tip-total"><span>闪避几率</span> <span>${dodgePct}%</span></div>
                </div>
            `;
            const dodgeSpan = `<span class="combat-tooltip-trigger" style="color:${color}; font-weight:bold; cursor:help; border-bottom:1px dotted #ccc; position:relative;">✨闪避${dodgeTooltip}</span>`;

            this._log(`${attackerName} 发起攻击，但是被 ${dodgeSpan} 了！`);
            return 0;
        }
        ...
        if (!isPlayerAttacking) {
            ...
            let tox = atkStats.toxAtk;
            console.log(`[Combat] 读取到的 toxAtk:`, tox);
            ...
            if (tox && Number(tox) > 0) {
                ...
                console.log("[Combat] ❌ 未触发中毒 (toxAtk 无效或为0)");
            }
            console.groupEnd();
        }
